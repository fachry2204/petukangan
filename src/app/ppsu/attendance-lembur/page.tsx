...
export default function PpsuAttendanceLemburPage() {
  ...
  const drawWatermark = (canvas: HTMLCanvasElement, timestamp: string, coordsStr: string, addrStr: string) => {
    ...
    ctx.fillText('SI PETUT ABSENSI PPSU (LEMBUR)', padding, height - bannerHeight + padding + fontSize);
  ...
  const submitAttendance = async () => {
    ...
    try {
      ...
      toast({ 
        ...
        description: isCheckOut ? 'Absensi keluar lembur Anda telah dicatat di database MySQL' : 'Kehadiran lembur Anda telah dicatat di database MySQL' 
      });
      setPhoto(null);
      fetchTodayStatus(false);
    } catch (error: any) {
