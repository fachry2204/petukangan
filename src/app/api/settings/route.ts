...
export async function POST(req: Request) {
  try {
    ...
    if (existing && existing.length > 0) {
      await queryDb(
        `UPDATE system_settings SET logoUrl=?, bgType=?, bgImage=?, bgVideo=?, bgVideoVolume=?, systemName=?, systemDescription=?, mainColor=?, maintenanceActive=?, maintenanceEnd=?, maintenanceTitle=?, maintenanceDesc=?, shifts=?, zones=?, updatedAt=NOW(6) WHERE id=?`,
        [data.logoUrl || '/logodki.png', data.bgType || 'image', data.bgImage || '/bg.jpg', data.bgVideo || '', data.bgVideoVolume ?? 0, data.systemName || 'SIPETUT', data.systemDescription || 'Monitoring & Management System', data.mainColor || '#f97316', data.maintenanceActive ? 1 : 0, data.maintenanceEnd || '', data.maintenanceTitle || 'Sistem Dalam Perbaikan', data.maintenanceDesc || 'Kami sedang melakukan pemeliharaan sistem. Silakan kembali lagi nanti.', shifts, zones, existing[0].id]
      );
    } else {
      await queryDb(
        `INSERT INTO system_settings (logoUrl, bgType, bgImage, bgVideo, bgVideoVolume, systemName, systemDescription, mainColor, maintenanceActive, maintenanceEnd, maintenanceTitle, maintenanceDesc, shifts, zones, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(6))`,
        [data.logoUrl || '/logodki.png', data.bgType || 'image', data.bgImage || '/bg.jpg', data.bgVideo || '', data.bgVideoVolume ?? 0, data.systemName || 'SIPETUT', data.systemDescription || 'Monitoring & Management System', data.mainColor || '#f97316', data.maintenanceActive ? 1 : 0, data.maintenanceEnd || '', data.maintenanceTitle || 'Sistem Dalam Perbaikan', data.maintenanceDesc || 'Kami sedang melakukan pemeliharaan sistem. Silakan kembali lagi nanti.', shifts, zones]
      );
    }
